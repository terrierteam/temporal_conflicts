...
class inference():
    def __init__(self, args):
        ...
        with open("aliases_MULAN.json", "r") as f:
            self.aliases = json.load(f)  # load() parses JSON into a Python dict
    ...
    def run(self):
        ...
        if self.args.context:
            filename = "../MULAN_Output/%s/%s_%s_context.json" % (self.args.mode,self.args.mode, self.args.model_name.split("/")[-1])
        else:
            filename = "../MULAN_Output/%s/%s_%s_q_only.json" % (self.args.mode,self.args.mode, self.args.model_name.split("/")[-1])
    ...
    def run_with_csv(self):
        ...
        for i in tqdm(range(len(self.data))):
            ...
            for gold_answer in answer_list:
                ...
                # if no context mode -> compare against all allowed answers (not just aliases)
                # if w/context mode -> compare against aliases of the target object
                # score = self.evaluate(answer, gold_answer)
                # score_list.append(score)

                if self.args.context:
                    allowed_answers = line["obj_aliases"]
                else:
                    allowed_answers = []
                    for ans in line["answer"]:
                        allowed_answers.append(ans["name"])
                        if ans["wikidata_id"] in self.aliases:
                            ans_aliases = self.aliases[ans["wikidata_id"]]
                            allowed_answers.extend(ans_aliases)
                score = self.max_over_allowed_answers(prediction=answer, allowed_answers=allowed_answers)
                score_list.append(score)
        ...
        to_save = {
            "args": vars(self.args),  # converts argparse.Namespace to a dict
            "accuracy": float(np.mean(score_list))  # convert numpy float to Python float (JSON safe)
        }

        # to save accuracy
        if self.args.context:
            filename = "../MULAN_Output/%s/%s_%s_context_accuracy.json" % (self.args.mode,self.args.mode, self.args.model_name.split("/")[-1])
        else:
            filename = "../MULAN_Output/%s/%s_%s_q_only_accuracy.json" % (self.args.mode,self.args.mode, self.args.model_name.split("/")[-1])
    ...
    def max_over_allowed_answers(self, prediction, allowed_answers):
        for ans in allowed_answers:
            score_dict = self.scorer.score(prediction, ans)
