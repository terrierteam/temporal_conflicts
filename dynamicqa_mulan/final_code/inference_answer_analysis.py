...
class inference():
    ...
    def run(self):
        ...
        if self.args.context:
            filename = "../Analysis_Output/%s/%s_%s_context_sampling_%s.json" % (self.args.mode,self.args.mode, self.args.model_name.split("/")[-1], self.args.do_sample)
        else:
            filename = "../Analysis_Output/%s/%s_%s_q_only_sampling_%s.json" % (self.args.mode,self.args.mode, self.args.model_name.split("/")[-1], self.args.do_sample)
    ...
    def run_with_json(self):
        ...
        for d in tqdm(self.data):
            for idx, ctx in enumerate(d["contexts"]):
                ...
                generated_ids = self.model.generate(encoded, max_new_tokens=64, do_sample=self.args.do_sample)
    ...
    def run_with_csv(self):
        ...
        for i in tqdm(range(len(self.data))):
            ...
            for gold_answer in answer_list:
                ...
                if "gpt" in self.args.model_name:
                    ...
                    encoded = self.tokenizer.apply_chat_template(input_msgs, return_tensors="pt").to(self.args.device)
                generated_ids = self.model.generate(encoded, max_new_tokens=20, do_sample=self.args.do_sample)
                decoded = self.tokenizer.batch_decode(generated_ids)
                # debug
                print(f"{decoded=}")
        ...
        # ✅ attach predictions as a new column
        self.data["prediction"] = output_list
        self.data["gold_obj"] = golden_list   # optional: keep gold answers too
        self.data["correct_original"] = score_list # optional: 1/0 accuracy for each row
        ...
        if self.args.context:
            fname = "../Analysis_Output/%s/%s_%s_context_analysis_sampling_%s.csv" % (self.args.mode,self.args.mode, self.args.do_sample,self.args.model_name.split("/")[-1], self.args.do_sample)
        else:
            fname = "../Analysis_Output/%s/%s_%s_q_only_analysis_sampling_%s.csv" % (self.args.mode,self.args.mode, self.args.model_name.split("/")[-1], self.args.do_sample)
