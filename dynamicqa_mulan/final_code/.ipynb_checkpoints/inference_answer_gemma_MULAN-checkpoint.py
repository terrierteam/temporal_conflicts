    ...
    def load_model(self):
        if self.args.bit4:
            ...
            if "gemma" in args.model_name:
                model = AutoModelForCausalLM.from_pretrained(args.model_name, torch_dtype=torch.bfloat16).to(self.args.device)
                print(f"Loaded {args.model_name} in BF16 format")
            else:
    ...
    def run_with_csv(self):
        ...
        for i in tqdm(range(len(self.data))):
            ...
            for gold_answer in answer_list:
                ...
                if self.args.context:
                    if "Qwen" in self.args.model_name or "llama" in self.args.model_name or "gemma-3-1b" in self.args.model_name:
                        system = {
                            "role": "system",
                    ...
                    elif "gemma-3-12b" in self.args.model_name or "gemma-3-4b" in self.args.model_name:
                        system = {
                            "role": "system",
                            "content": [{"type": "text", "text": "You'll be given a question and a context about the article and answer it with a one word. Answer the [Question]."}]
                        }
                        user = {
                            "role": "user",
                            "content": [{"type": "text", "text": "[Context]: %s [Question]: %s [Answer]:" % (
                                context, line["question"])}]
                        }
                    else:
                        user = {
                    ...
                    if "Qwen" in self.args.model_name or "llama" in self.args.model_name or "gemma-3-1b" in self.args.model_name:
                        ...
                        system = {
                            "role": "system",
                            "content": [{"type": "text", "text": "You'll be given a question about the article and answer it with a one word. Answer the [Question]. "}]
                        }
                        user = {
                            "role": "user",
                            "content": [{"type": "text", "text": "This article is about %s. [Question]: %s [Answer]:" % (
                                line["subj"], line["question"])}]
                        }                        
                    else:
                        user = {
                ...
                if "Qwen" in self.args.model_name or "llama" in self.args.model_name or "gemma" in self.args.model_name:
                ...
                if "gpt" in self.args.model_name:
                    ...
                    encoded = self.tokenizer.apply_chat_template(input_msgs, return_tensors="pt", add_generation_prompt=True).to(self.args.device)
                else:
                    encoded = self.tokenizer.apply_chat_template(input_msgs, return_tensors="pt").to(self.args.device)
        ...
        # Make model name filename-safe
        model_safe = self.args.model_name.replace("/", "_")
        context_flag = "context" if self.args.context else "no_context"
        
        # Save summary JSON in same folder as script
        summary_filename = f"{model_safe}_{self.args.mode}_{context_flag}_summary.json"
        
        with open(summary_filename, "w") as f:
            json.dump({
                "model": self.args.model_name,
                "mode": self.args.mode,
                "context": bool(self.args.context),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["mutable", "immutable", "immutable_n"],
        required=True,
        help="Mode must be one of: mutable, immutable, immutable_n"
    )
