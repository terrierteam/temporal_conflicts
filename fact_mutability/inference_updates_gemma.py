...
from inference_gemma import get_generation_config, get_scores, prepare_prompt
...
def main(args):
    ...
    ds = load_from_disk(f"dataset/{args.model_name}_updates")
    print(ds)
    ...
    for mutability in list(
        ...
        result_dict = {}
        result_dict[f"{mutability}_total"] = total
        result_dict[f"{mutability}_succ"] = succ
        result_dict[f"{mutability}_succ_rate"] = succ / total
        print(result_dict)
        results_fname = f"rq3result_{args.model_name}_{mutability}.json"
        with open(results_fname, "w") as json_file:
