...
def prepare_prompt(query, model_name_or_path, instruction, tokenizer, template=None, add_generation_prompt=True):
    ...
    elif "gemma-3-4b" in model_name_or_path or "gemma-3-12b" in model_name_or_path:
        # system = "Complete the fact in as few words as possible"
        messages = [{
                "role": "system",
...
def get_scores(model_output, input_ids, prompt, query, tokenizer):
    ...
    # # debug
    # get_sequence_output = tokenizer.decode(sequence).strip()
    # print(f"{get_sequence_output=}")
    ...
    for line in tqdm(dataset):
        ...
        with torch.no_grad():
            prompt = prepare_prompt(
                query, args.model_name_or_path, args.instruction, tokenizer, args.template 
            )
            # # debug line
...
def main(args):
    ...
    if "gemma" in args.model_name_or_path:
        model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, torch_dtype=torch.bfloat16).to(
                device
            )
    else:
        model = AutoModelForCausalLM.from_pretrained(args.model_name_or_path, torch_dtype=torch.float16).to(
            device
        )
    model.eval()
