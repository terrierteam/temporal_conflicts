...
with open("dqa_rel_dict.json", "r") as f:
    sorted_relations_by_type = json.load(f)
...
def main():
    ...
    #     ]
    # models = [
    #     ("gemma1b", "output/fm_queries_{}_exp--google-gemma-3-1b-it/predictions.json"),
    #     ("gemma4b", "output/fm_queries_{}_exp--google-gemma-3-4b-it/predictions.json"),
    #     ("gemma12b", "output/fm_queries_{}_exp--google-gemma-3-12b-it/predictions.json"),
    # ]

    models = [
        # ("llama_dqa", "dqa_output/fm_queries_{}_exp--meta-llama-Llama-2-7b-hf/predictions.json"),
        # ("llamachat_dqa", "dqa_output/fm_queries_{}_exp--meta-llama-Llama-2-7b-chat-hf/predictions.json"),
        ("falconbase_dqa", "dqa_output/fm_queries_{}_exp--tiiuae-falcon-7b/predictions.json"),
        ("falconinstruct_dqa", "dqa_output/fm_queries_{}_exp--tiiuae-falcon-7b-instruct/predictions.json"),
    ]
    ...
    for m, _ in models:
        ...
        for t in templates:
            with open(f"./dqa_output/{m}_{t}/metrics.jsonl") as fhandle:
                for line in fhandle:
    ...
    for m, pred_file in models:
        ...
        for type, rels in sorted_relations_by_type.items():
            ...
            for rel in rels:
                ...
                subjects = json.load(open("./data/wikidata/dqa_objects_by_freq/{}.json".format(rel)))
                stats = pd.read_json(f"./dqa_output/{m}_{template}/{rel}_results_per_example.json")
