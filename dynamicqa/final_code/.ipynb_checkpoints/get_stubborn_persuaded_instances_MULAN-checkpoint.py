...
def find_stubborn_example(model_name, mode):

    # wrong with context but the same answer with the parametric knowledge
    context_file = "../CP_Output/sequences/%s/%s_%s_context.json" %(mode,mode, model_name)
    q_only_file = "../CP_Output/sequences/%s/%s_%s_q_only.json" %(mode,mode, model_name)
...
def persuasion_analysis(model_name, stub_list, pers_list, mode):
    result = pickle.load(open("../CP_Output/sequences/%s/%s_%s_persuasion_output.pkl" % (mode, mode, model_name), "rb"))
    print(len(result))
    data = load_reference(mode)
...
if __name__ == "__main__":
    ...
    for short_name, long_name in model_list.items():
        for mode in ["mutable", "immutable", "immutable_n"]:
            ...
            pickle.dump(stub_pers, open("../CP_Output/final/%s_%s_persuasion_stubborn.pkl" % (mode, short_name), "wb"))

            print("\n# of Persuaded", len(persuaded_list))
            print("Persuasion Score on Persuaded", np.mean(pers_pers))
            pickle.dump(pers_pers, open("../CP_Output/final/%s_%s_persuasion_persuaded.pkl" % (mode, short_name), "wb"))
            print()
