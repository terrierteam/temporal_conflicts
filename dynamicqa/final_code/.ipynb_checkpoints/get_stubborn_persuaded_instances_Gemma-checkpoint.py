...
def find_stubborn_example(model_name, mode="static"):
    ...
    context_file = "../Gemma_Output/sequences/%s/%s_%s_context.json" %(mode,mode, model_name)
    q_only_file = "../Gemma_Output/sequences/%s/%s_%s_q_only.json" %(mode,mode, model_name)
...
def persuasion_analysis(model_name, stub_list, pers_list, mode="static"):
    result = pickle.load(open("../Gemma_Output/sequences/%s/%s_%s_persuasion_output.pkl" % (mode, mode, model_name), "rb"))
    print(len(result))
    data = load_reference(mode)
...
if __name__ == "__main__":
    ...
    # nfs/projects/dynamicqa/Gemma_Output/sequences/static/static_gemma-3-12b-it_context.json

    # stubborn example count
    # model_list = {"Qwen2":"Qwen2-7B-Instruct", "Llama":"Llama-2-7b-chat-hf", "Mistral":"Mistral-7B-Instruct-v0.1"}
    model_list = {"Gemma1B":"gemma-3-1b-it", "Gemma4B":"gemma-3-4b-it", "Gemma12B":"gemma-3-12b-it"}
    ...
    for short_name, long_name in model_list.items():
        for mode in ["temporal", "static", "dispute"]:
            ...
            pickle.dump(stub_pers, open("../Gemma_Output/final/%s_%s_persuasion_stubborn.pkl" % (mode, short_name), "wb"))

            print("\n# of Persuaded", len(persuaded_list))
            print("Persuasion Score on Persuaded", np.mean(pers_pers))
            pickle.dump(pers_pers, open("../Gemma_Output/final/%s_%s_persuasion_persuaded.pkl" % (mode, short_name), "wb"))
            print()

            print("\n# of Contradict correct", len(contradict_correct))
            print()
