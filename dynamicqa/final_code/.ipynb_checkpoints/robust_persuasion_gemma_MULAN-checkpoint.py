...
class cmConflict():
    ...
    def run(self):
        ...
        score_filename = "%s_%s_persuasion_score.json" % (
            self.args.mode,
            self.args.model_name.split("/")[-1]
        )
        with open(score_filename, "w") as f:
            json.dump({
                "mean_persuasion_score": float(np.mean(total_pers)),
                "all_scores": [float(x) for x in total_pers]  # make sure numpy floats are converted
            }, f, indent=2)
    ...
    def prepare_input(self, line, answer, context=False):
        ...
        if self.args.mode == "dispute":
            ...
            ctx = line["context"].replace("[ENTITY]", answer)
        if context:
            if "Qwen" in self.args.model_name or "gemma-3-1b" in self.args.model_name:
                system = {
                    "role": "system",
        ...
        if "Qwen" in self.args.model_name or "gemma" in self.args.model_name:
